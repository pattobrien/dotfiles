...
{/_ Only the adapter package + chat (+ optional state adapter). Extra imports
used below go in code blocks, NOT here — see the peerDeps invariant. _/}
...
<TypeTable
  type={{
    ...
    webhookSecret: {
      type: "string",
      description:
        "Signing secret for inbound webhook verification. Falls back to `FOO_WEBHOOK_SECRET`.",
    },
  }}
/>
...
{/_ Add the sections that apply: sending, reactions, attachments, thread IDs,
limitations, links. Keep it faithful to the adapter's real behavior. _/}
