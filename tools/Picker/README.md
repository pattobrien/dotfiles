...
A native macOS floating picker. Reads items from stdin, presents a searchable
list in a borderless floating window, and prints the selected item to stdout.

Built to avoid the flicker that terminal-based pickers (e.g. fzf in Kitty)
produce when dynamically resizing windows.
...
The binary is at `.build/release/picker` (or `.build/debug/picker` for debug
builds).
...
`make dev` pipes default test items into the picker and restarts it
automatically on each save. To use custom test data:
