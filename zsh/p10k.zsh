# Generated by Powerlevel10k configuration wizard on 2023-05-23 at 21:41 EDT.
...
() {
  ...
  # For truncate_before_marker=last, only use .git so that worktree roots are
  # the truncation point (not subdirs with package.json, go.mod, etc.).
  ...
  function my_git_formatter() {
    ...
    if (( $1 )); then
      ...
      local      clean='%151F'  # green foreground (Catppuccin Mocha green)
      local   modified='%216F'  # peach foreground (Catppuccin Mocha peach)
      local  untracked='%147F'  # lavender foreground (Catppuccin Mocha lavender)
      local conflicted='%211F'  # red foreground (Catppuccin Mocha red)
    else
    ...
    # Branch/tag/commit name hidden to save space (worktree name in dir is sufficient).
    # Show commit hash in detached HEAD state (no branch, no tag) as a fallback.
  ...
  typeset -g POWERLEVEL9K_BACKGROUND_JOBS_FOREGROUND=111
  # Custom icon.
